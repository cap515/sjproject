...
public class BoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 게시글 등록용 Controller
			...
			// 기존 처럼 request.getParameter() 를 이용하여
			// 입력값을 전달 받을 수 없음
			...
			try {
				// 요청(request)에 multipart/form-data 가 포함 되어 있는가?
				if(ServletFileUpload.isMultipartContent(request)) {
					...
					String savePath = root + "resources/uploadImgs/";
					
					// 2. 파일명 변환 작업
					// 파일명 중복으로 인한 데이터 손실 예방
					// + 파일명이 한글 또는 특수문자가 포함 되면 서버 종류에 따라 문제가 발생할 수 있다.
					
					// cos.jar -> DefaultFileRenamePolicy 를 제공해 주지만
					// 나만의 파일명 규칙을 적용하기 위해서 별도의 MyFileRenamePolicy 작성
					
					// 3. MultipartRequest 객체 생성
					//	-> 객체가 생성되는 순간 request, 파일경로 지정, 
					//		최대 파일 크기 지정, 문자 인코딩 지정, * 지정된 경로에 변경된 파일명으로 파일이 저장됨
					MultipartRequest multiRequest = 
							new MultipartRequest(request, savePath, maxSize, "UTF-8", new MyFileRenamePolicy());
					
					// 4. 서버에 저장된 파일의 원래 파일 명, 이전 파일명을
					...
					// session은 request 에서만 얻어 올 수 있다.
					Member loginMember = (Member)request.getSession().getAttribute("loginMember");
					
					int boardWriter = loginMember.getMemberNo();
					
					// 6. Attachment VO를 생성한 후
					// 	  Attachment들을 저장 할 list를 생성하여
					//    파일 경로, 파일 원본명, 변경된 파일명을 세팅하여 전달
		...
		// 게시글 상세조회 Controller
			...
			try {
				...
				if( board!=null ) {
					...
					int result = new BoardService().boardCountPlus(boardNo);
					if(!files.isEmpty()) {
			...
			try {
				...
				if(file != null) {
					...
					int result = new BoardService().downloadCountPlus(fNo);
					int i = 1 / result;
